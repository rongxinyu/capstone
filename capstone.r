...
hybridScheme <- function(params){
    ...
    Simulation <- function(n, kappa, T){
       #W <- mvrnorm(floor(n*T), mu=rep(0, kappa+1), Sigma=sigma)
       #Wperp <- rnorm(floor(n*T))
       #Z <- rho * W[,1] + sqrt(1-rho*rho)*Wperp
       
       #Gamma <- sapply(seq(1:floor(n*T)), function(x){(floor(n*T)/n-bstar(x, alpha)/n)^alpha}) # not sure if I used the right g() function
       #Gamma[1:kappa] <- 0
...
finalPrices <- hybridScheme(params)(100, 1, 1)
